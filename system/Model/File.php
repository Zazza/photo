...
class Model_File extends Engine_Model {
	...
	function getAdminFiles($curdir) {
		$sql = "SELECT DISTINCT(f.id), f.md5, f.filename AS `name`, f.size, h.uid, h.timestamp, r.right AS `right`, f.close AS `close`
				...
				LEFT JOIN fm_fs_history AS h ON (h.fid = f.id)
				LEFT JOIN fm_fs_chmod AS r ON (r.fid = f.id)
	...
	function getAdminFilesSort($curdir) {
		...
		if ( (isset($sphoto["tag"])) and (count($sphoto["tag"]) > 0) ) {
			...
			foreach($sphoto["tag"] as $part) {
				...
				$left_inc .= " LEFT JOIN fm_photo_tags AS fpt" . $i . " ON (fpt" . $i . ".fid = f.id) ";
		...
		if ( (isset($sphoto["sel"])) and (count($sphoto["sel"]) > 0) ) {
			foreach($sphoto["sel"] as $part) {
				...
				$left_inc .= " LEFT JOIN fm_photo_desc AS fpd" . $i . " ON (fpd" . $i . ".fid = f.id) ";
	...
	function getAdminFilesFavorite($curdir) {
		...
		LEFT JOIN fm_fs_history AS h ON (h.fid = f.id)
		LEFT JOIN fm_fs_chmod AS r ON (r.fid = f.id)
		LEFT JOIN fm_favorite AS ff ON (ff.fid = f.id)
	...
	function delfile($md5) {
		$sql = "UPDATE fm_fs SET `close` = '1' WHERE `md5` = :md5 LIMIT 1";
	...
	function delfilereal($md5) {
		...
		if (file_exists($this->registry["rootPublic"] . $this->registry["path"]["upload"] . $md5)) {
			unlink($this->registry["rootPublic"] . $this->registry["path"]["upload"] . $md5);
			unlink($this->registry["rootPublic"] . $this->registry["path"]["upload"] . "_thumb/" . $md5);
		}

		$sql = "DELETE FROM fm_fs WHERE `md5` = :md5 LIMIT 1";
	
		$res = $this->registry['db']->prepare($sql);
		$param = array(":md5" => $md5);
		$res->execute($param);
		
		$sql = "DELETE FROM fm_fs_chmod WHERE `fid` = :fid LIMIT 1";
		
		$res = $this->registry['db']->prepare($sql);
		$param = array(":fid" => $data["id"]);
		$res->execute($param);
		
		$sql = "DELETE FROM fm_fs_history WHERE `fid` = :fid LIMIT 1";
		
		$res = $this->registry['db']->prepare($sql);
		$param = array(":fid" => $data["id"]);
		$res->execute($param);
		
		$sql = "DELETE FROM fm_favorite WHERE `fid` = :fid LIMIT 1";
		
		$res = $this->registry['db']->prepare($sql);
		$param = array(":fid" => $data["id"]);
		$res->execute($param);
		
		$sql = "DELETE FROM fm_photo_desc WHERE `fid` = :fid LIMIT 1";
		
		$res = $this->registry['db']->prepare($sql);
		$param = array(":fid" => $data["id"]);
		$res->execute($param);
		
		$sql = "DELETE FROM fm_photo_tags WHERE `fid` = :fid LIMIT 1";
	...
	function moveFiles($curdir, $md5, $buffer) {
		$sql = "UPDATE fm_fs SET pdirid = :dir WHERE `md5` = :md5 AND pdirid = :curdir AND close = 0";
	...
	function getUsersChmod($md5) {
		$sql = "SELECT r.right AS `right`
	        FROM fm_fs AS fs
	        LEFT JOIN fm_fs_chmod AS r ON (r.fid = fs.id)
	        WHERE fs.md5 = :md5
	        LIMIT 1";
	...
	function restoreFile($md5) {
		$sql = "UPDATE fm_fs SET `close` = '0' WHERE md5 = :md5 LIMIT 1";
	...
	function fileRename($md5, $newname) {
		$sql = "UPDATE fm_fs SET filename = :newname WHERE md5 = :md5 LIMIT 1";
