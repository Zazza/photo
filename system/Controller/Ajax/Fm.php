    ...
    function files() {
		...
		for($i=0; $i<count($files); $i++) {
			...
			if ($flag) {
				...
				$res_files[$k]["ico"] = $this->registry["path"]["upload"] . "_thumb/" . $files[$i]["md5"];
        ...
        echo $this->view->render("fm_content", array("admin" => $fm["admin"], "clip" => $clip, "shPath" => $shPath, "dirs" => $res_dirs, "_thumb" => $this->registry['path']['upload'] . "_thumb/", "files" => $res_files, "totalsize" => $total, "curdir" => $this->_curdir, "fsRes" => $this->view->render("fm_view", array())));
    }
